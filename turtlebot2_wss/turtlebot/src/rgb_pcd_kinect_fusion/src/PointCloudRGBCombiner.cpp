...
public:
    ...
    PointCloudRGBCombiner(ros::NodeHandle nh): nh_(nh), n("~") // ,colored_pcl_in(0L)
    {
        // std::string image_topic = nh_.resolveName(“img_in”);
        // std::string image_topic = "/rgb/image_rect_color";
        image_topic = getParam<std::string>(n, "image_topic", "/rgb/image_rect_color"); 
        pcl_topic = getParam<std::string>(n, "pcl_topic", "/depth/pcd");
        pcd_color_topic = getParam<std::string>(n, "pcd_color_topic", "/depth/pcd_colored");
        pcd_rect_topic = getParam<std::string>(n, "pcd_rect_topic", "/depth/pcd_rect");
        far_clip = getParam<double>(n, "far_clip", 3.5);
    ...
    void imageCb(const sensor_msgs::ImageConstPtr& msg){
        ...
        // while (ros::ok()){
            ...
            sensor_msgs::PointCloud2::ConstPtr recent_cloud = ros::topic::waitForMessage<sensor_msgs::PointCloud2>(pcl_topic, nh_, ros::Duration(1.0));
...
int main(int argc, char **argv){
    ros::init(argc, argv, "rgb_pcd_kinect_fusion");
    ros::NodeHandle nh;
