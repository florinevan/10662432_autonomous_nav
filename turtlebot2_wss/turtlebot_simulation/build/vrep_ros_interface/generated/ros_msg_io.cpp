...
void write__nifti_robot_driver_msgs__TracksStamped(const nifti_robot_driver_msgs::TracksStamped& msg, int stack, const WriteOptions *opt)
{
    try
    {
        simPushTableOntoStackE(stack);
        ...
        {
            // write field 'right'
            simPushStringOntoStackE(stack, "right", 0);
            write__float64(msg.right, stack, opt);
            simInsertDataIntoStackTableE(stack);
        }
        catch(exception& ex)
        {
            std::string msg = "field 'right': ";
            msg += ex.what();
            throw exception(msg);
        }
        try
        {
            // write field 'left'
            simPushStringOntoStackE(stack, "left", 0);
            write__float64(msg.left, stack, opt);
            simInsertDataIntoStackTableE(stack);
        }
        catch(exception& ex)
        {
            std::string msg = "field 'left': ";
            msg += ex.what();
            throw exception(msg);
        }
    }
    ...
    {
        std::string msg = "write__nifti_robot_driver_msgs__TracksStamped: ";
        msg += ex.what();
        throw exception(msg);
    }
}

void read__nifti_robot_driver_msgs__TracksStamped(int stack, nifti_robot_driver_msgs::TracksStamped *msg, const ReadOptions *opt)
{
    try
    {
        int r = simGetStackTableInfoE(stack, 0);
        ...
        while(numItems >= 1)
        {
            ...
            if((str = simGetStackStringValueE(stack, &strSz)) != NULL && strSz > 0)
            {
                ...
                else if(strcmp(str, "right") == 0)
                {
                    ...
                    {
                        // read field 'right'
                        read__float64(stack, &(msg->right), opt);
                    }
                    catch(exception& ex)
                    {
                        std::string msg = "field right: ";
                ...
                else if(strcmp(str, "left") == 0)
                {
                    ...
                    {
                        // read field 'left'
                        read__float64(stack, &(msg->left), opt);
                    }
                    catch(exception& ex)
                    {
                        std::string msg = "field left: ";
    ...
    {
        std::string msg = "read__nifti_robot_driver_msgs__TracksStamped: ";
        msg += ex.what();
        throw exception(msg);
    }
}
...
void ros_callback__nifti_robot_driver_msgs__TracksStamped(const boost::shared_ptr<nifti_robot_driver_msgs::TracksStamped const>& msg, SubscriberProxy *proxy)
{
    int stack = -1;
    try
    {
        stack = simCreateStackE();
        write__nifti_robot_driver_msgs__TracksStamped(*msg, stack, &(proxy->wr_opt));
        simCallScriptFunctionExE(proxy->topicCallback.scriptId, proxy->topicCallback.name.c_str(), stack);
        simReleaseStackE(stack);
        stack = -1;
    }
    catch(exception& ex)
    {
        if(stack != -1)
            simReleaseStack(stack); // don't throw
        std::string msg = "ros_callback__nifti_robot_driver_msgs__TracksStamped: ";
        msg += ex.what();
        simSetLastError(proxy->topicCallback.name.c_str(), msg.c_str());
    }
}
